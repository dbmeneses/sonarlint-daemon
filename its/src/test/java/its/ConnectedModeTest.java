...
public class ConnectedModeTest {
  ...
  public void testNormal() throws InterruptedException, IOException {
    ...
    // REGISTER
    sonarlint.register(createConnectedConfig());

    // STATE
    assertThat(sonarlint.status(Void.newBuilder().build()).getStatus()).isEqualTo(StorageStatus.Status.NEVER_UPDATED);
    ...
    // STATE
    assertThat(sonarlint.status(Void.newBuilder().build()).getStatus()).isEqualTo(StorageStatus.Status.UPDATED);

    // UPDATE MODULE
    ModuleUpdateReq moduleUpdate = ModuleUpdateReq.newBuilder()
      .setModuleId(PROJECT_KEY_JAVA)
  ...
  private ClientCall<Void, LogEvent> getLogs(LogCollector collector, Channel channel) {
    ClientCall<Void, LogEvent> call = channel.newCall(StandaloneSonarLintGrpc.METHOD_LOG, CallOptions.DEFAULT);
